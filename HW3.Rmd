...
*The marginal externality cost per gallon of gasoline is `$``r round(t, 2)` and found by $51 per metric ton/2204.62 lbs per metric ton x 19.6 lbs per gallon gas.*
...
benefit_table <- tibble_base %>% 
  round(digits = 2) %>% 
  rename('SCC ($/MTCO2eq.)' = 'x', 
         'Tax Rate ($)' = 'y <- (x/2204.62) * 19.6',
         'Pre Distribution High ($)' = 'q <- ((a_high - (3 + y)) * (demand((3 + y), model_demand_high))/2)',
         'Pre Distribution Low ($)' = 'z <- ((a_low - (3 + y)) * (demand((3 + y), model_demand_low))/2)',
         'Post Distribution High ($)' = 'v <- q + demand_alt(3 + y) * y * pretax_perc_high',
         'Post Distribution Low ($)' = 'w <- z + demand_alt(3 + y) * y * (1/pretax_perc_high)',
         'Producer Surplus ($)' = 'p <- producer_surplus(x)')
