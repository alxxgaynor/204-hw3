...
a_low_invert <- demand(0,model_demand_low)
a_high_invert <- demand(0,model_demand_high)
```
...
```{r, echo = FALSE, message = FALSE, warning = FALSE}
## Plot in agg demand and supply
plot_e <- plot_2 + xlim(0, 800000) +
  ylim(0,16) +
  geom_abline(intercept=a_tot, slope=b_tot, color = "green") +
  geom_abline(intercept=0, slope=s_slope, color = "blue")
```

# 2 Curves
### What is the aggregate daily demand curve for gasoline?
### From $0 to `r round(a_low,2)`: P = `r round(b_tot,7)`Q + `r round(a_tot,1)`, From `r round(a_low,1)` to infity:  P = `r round(b_high,7)`Q + `r round(a_high,1)`  
...
# 3 Consumer Benefit
###  "high" income consumers attain `r round(social_high,1)` and "Low" income consumers  attain `r round(social_low,1)`, 

# 4
## A
### The amount of gasloine produced and consumed will move from `r demand_alt(3)` down to `r demand_alt(3.45)`
