...
**"the same as the electric car technology" ?must? mean (environmental value per gallon saved: $0.45 if the SCC is $51)x(gallons used after some rich people switch to electric) = `r round(e_tax_revenue,0)`. In order to generate the same amount via a tax on the remaining gallons of gas consumed, you would divide the amount by the miles still driven and get `r round(equal_gas_tax,2)`.**
