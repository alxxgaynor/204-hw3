...
# Mutating a table via tibble:

<<<<<<< HEAD
tibble_base <- tibble(x = c(51,75,100,125,150),
=======
benefit_table <- tibble(x = c(51,
                            75,
                            100,
                            125,
                            150),
>>>>>>> 7aa9e9683ea4871f86be6d16a11f8b19bdd02675
                      y <- (x/2204.62)*19.6,
  ...
  round(digits = 2) %>% 
  rename("SCC ($/MTCO2eq.)" = "x") %>% 
  rename("Tax Rate ($)" = "y <- (x/2204.62) * 19.6") %>% 
  rename("Pre Distribution High ($)" = "q <- ((a_high - (3 + y)) * (demand((3 + y), model_demand_high))/2)") %>% 
  rename("Pre Distribution Low ($)" = "z <- ((a_low - (3 + y)) * (demand((3 + y), model_demand_low))/2)") %>% 
  rename("Post Distribution High ($)" = "v <- q + demand_alt(3 + y) * y * pretax_perc_high") %>% 
  rename("Post Distribution Low ($)" = "w <- z + demand_alt(3 + y) * y * (1/pretax_perc_high)") %>% 
  rename("Producer Surplus ($)" = "p <- producer_surplus(x)")
...
```{r}
# clean_benefit_tible <- benefit_tible %>%
#   rename( y = "y <- (x/2204.62)*19.6")
  #         q = q <- ((a_high-(3+y))*(demand((3+y),
  #                              model_demand_high))/2),
  #         z = z <- ((a_low-(3+y))*(demand((3+y),
  #                             model_demand_low))/2),
  #         v = v <- q + demand_alt(3+y)*y*pretax_perc_high,
  #         w = w <- z+demand_alt(3+y)*y*(1/pretax_perc_high),
  #         v = p <- producer_surplus(x))

# ggplot(benefit_tible, aes() +
#   geom_histogram() +
...
```{r, echo = FALSE, warning = FALSE, message = TRUE, results = TRUE}
benefit_kbl <- kbl(benefit_table,
                   col.names = c("SCC ($/MTCO2eq.)",
                "Tax Rate ($)",
                "Pre Distribution High ($)",
                "Pre Distribution Low ($)",
